...
| **Test** | Deduped coordinate row: plain `project` / `kind` / `mode` / `os` / `os_version` / `distro` / `distro_version` / `flavor` / `flavor_version` / `arch` / `compiler` / `compiler_version` / `isolation` / `toolchain` / `opp_file` string columns (no FK to Project / OS / Compiler — denormalised by design, so a run record survives catalog edits) keyed by SHA-256 `coord_hash`. The only mutable column is `name`; expectations live in their own audited table (see below). |
| **ExpectedTestResult** | Append-only edit log of expected outcomes per Test: test FK, `expected_result_code` (PASS/FAIL/ERROR/SKIPPED or NULL = retraction), `expected_result_description`, `reason`, `set_by`, `set_at`. "Current expectation" is the row with the highest `set_at`; nothing is ever updated. |
| **TestMatrixRun** | One row per matrix submission: matrix FK, `trigger` (manual/cli/web/remote/webhook/tag/schedule/rerun), `ref` (tag name for tag triggers), `github_*` linkage fields, `created_at`, `completed_at`. Plus a stored, transactional rollup updated as each child cell finalizes: per-result counters (`pass_count`/`fail_count`/`error_count`), per-verdict counters (`expected_count`/`unexpected_count`/`unknown_count`), `cache_hit_count`, `total_count`, `actual_summary` (worst outcome across cells), and `verdict` (EXPECTED/UNEXPECTED/UNKNOWN). |
| **TestRun** | One attempt at a Test: test FK, optional matrix_run FK, worker FK, `git_ref`, `commit_sha`, `version`, `resolved_deps`, `lifecycle` (queued/running/finished/cancelled/timed_out), timestamps, and — populated iff lifecycle=finished — outcome columns `result_code` (PASS/FAIL/ERROR/SKIPPED), `stdout`/`stderr` (raw with ANSI), free-form `details` JSON. Also `system_snapshot` JSON captured at run start, and a `cache_fingerprint` (SHA-256 over coords + resolved ref + deps) consulted by the content-addressable cache. |
| **TestVerdict** | One row per cell of a TestMatrixRun: matrix_run FK, test FK, test_run FK (a fresh row on miss, a pre-existing finished row on cache hit), nullable expectation FK (pins the ExpectedTestResult in force at recording time), `verdict` (EXPECTED/UNEXPECTED/UNKNOWN, NULL while the underlying run hasn't finalized), `recorded_at`, `created_at`, `cache_hit` flag. Cell lifecycle is derived from the underlying TestRun — not stored. |
